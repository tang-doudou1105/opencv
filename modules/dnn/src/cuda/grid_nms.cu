...
namespace raw {
    ...
    {
        struct BoundingBox { float xmin, ymin, xmax, ymax; };

        const auto compute_size = [](BoundingBox bbox)->float {
         ...
         *
         * We arbitarily choose a scheme to exit if group_i is greater than group_j. This way we only compute IOUs
         * between groups once. While nearly half the blocks are wasted, it's ok since they exit early on and the
         * working blocks are compute heavy.
         */
        if (group_i > group_j)
            return;
        ...
        /* MAIN LOOP LOGIC:
         * We compare a box `i` from group_i with all boxes (one per thread) in group_j in each iteration.
         * Each thread of the block computes the overlap between box `i` and its corresponding
         * box `j`. The `j` is exactly mapped to the thread index. Hence, the `j` is a loop invariant.
         *
         ...
         * We store the `i` boxes in shared memory to avoid multiple requests for the same box.
         */
        using vector_type = get_vector_type_t<T, 4>;
        __shared__ vector_type group_i_boxes[BLOCK_SIZE];

        /* The main loop is compute heavy. We will precompute the sizes of `i` boxes in the code
         * where we load them. This will ease the workload in the main loop and also help hide memory
         * access latency while loading boxes from global memory.
         */
        ...
        // load `i` boxes and precompute their sizes
        {
            ...
            if (group_i_offset + i < boxes)
            {
                ...
                group_i_size[i] = compute_size(bbox);
            }
        }
        ...
        if (group_j_offset + threadIdx.x < boxes)
        {
            ...
            bbox_j_size = compute_size(bbox_j);
        }
